...
exports.addToWishlist = catchAsyncErrors(async(req, res, next) => {
    ...
    try{
        ...
        if(existingProduct.length === 0){
            ...
            const [wishlistProducts] = await pool.execute("SELECT p.* FROM products p JOIN wishlist w ON p.id = w.product_id WHERE user_id = ?", [userId])
            res.status(201).json({
                success: true,
...
exports.removeFromWishlist = catchAsyncErrors(async(req, res, next) => {
    ...
    try{
        const [result] = await pool.execute("DELETE FROM wishlist WHERE user_id = ? AND product_id = ?", [userId, product_id])
        const [wishlistProducts] = await pool.execute("SELECT p.* FROM products p JOIN wishlist w ON p.id = w.product_id WHERE user_id = ?", [userId])
