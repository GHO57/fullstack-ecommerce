...
const CategorizedProducts = React.lazy(() => import('./components/user/CategorizedProducts/CategorizedProducts'));
...
const SellerProducts = React.lazy(() => import('./components/seller/SellerProducts/SellerProducts'));
const AddProduct = React.lazy(() => import('./components/seller/AddProduct/AddProduct'));
const DeletedProducts = React.lazy(() => import('./components/seller/DeletedProducts/DeletedProducts'));
const ManageOrdersSeller = React.lazy(() => import('./components/seller/ManageOrdersSeller/ManageOrdersSeller'));
const SellerOrderItems = React.lazy(() => import('./components/seller/SellerOrderItems/SellerOrderItems'));
...
const ManageSellerApplications = React.lazy(() => import('./components/admin/ManageSellerApplications/ManageSellerApplications'));
...
function AppContent() {
  ...
  const { wishlistLoading, wishlistMessage, wishlistError } = useSelector((state) => state.wishlist)

  const location = useLocation();
  const showHeaderFooter = !location.pathname.startsWith('/seller') && !location.pathname.startsWith('/admin');

  const renderSidebar = () => {
    if (location.pathname.startsWith('/seller') && location.pathname !== '/seller' && location.pathname !== '/seller/login' && isSellerAuthenticated) {
      return <SellerSidebar />;
    }
    if (location.pathname.startsWith('/admin') && location.pathname !== '/admin/login' && isAdminAuthenticated) {
      return <AdminSidebar />;
    }
    return null;
  };
  ...
  useEffect(() => {
    ...
    if((!loadingLogin && message) || (!loading && message) || (!adminLoading && adminMessage) || (!sellerLoading && sellerMessage) || (!cartLoading && cartMessage) || (!wishlistLoading && wishlistMessage)){
      toast.dismiss()
      toast.success(adminMessage || sellerMessage || message || cartMessage || wishlistMessage);
    ...
    }else if((!loadingLogin && error)   || (!loading && error) || (!adminLoading && adminError) || (sellerError && !sellerLoading) || (!cartLoading && cartError) || (!wishlistLoading && wishlistError)){
      if(adminError){
        toast.dismiss()
  ...
  }, [dispatch, loading, adminLoading, loadingLogin, message, adminMessage, error, adminError, sellerLoading, sellerMessage, sellerError, cartLoading, cartMessage, cartError, wishlistLoading, wishlistMessage, wishlistError])
      ...
      <div className={showHeaderFooter ? 'mt-[87px]' : 'mt-[47px]'}>
        <Suspense fallback={<Loader />}>
          <Routes>
            ...
            <Route exact path='/wishlist' element={isAuthenticated ? <Wishlist /> : <NotFound />} />
